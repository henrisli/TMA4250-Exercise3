...
\begin{cases}
\phi_1(d_i; 0.02, 0.06^2) \ &\textrm{if} \ l_i = 0 \\
\phi_1(d_i; 0.08, 0.06^2) \ &\textrm{if} \ l_i = 1
\end{cases} \\
&= \prod_{i : l_i = 0} \phi_1(d_i; 0.02, 0.06^2) \prod_{i:l_i = 1} \phi_1(d_i; 0.08, 0.06^2).
\end{split}
\label{eq:lik}
\end{equation}
...
\begin{equation}
\begin{split}
p(\mathbf{l} | \mathbf{d}) &= \frac{ \prod_{i : l_i = 0} \phi_1(d_i; 0.02, 0.06^2) \prod_{i:l_i = 1} \phi_1(d_i; 0.08, 0.06^2) }{\sum_{\mathbf{k} \in \{0, 1\}^n} \prod_{i : k_i = 0} \phi_1(d_i; 0.02, 0.06^2) \prod_{i:k_i = 1} \phi_1(d_i; 0.08, 0.06^2) } \\
...
```{r, echo = F, eval = T, fig.align = "center", out.width = "40%", fig.cap = "One realisation of the posterior RF with constant prior"}
...
```{r, echo = F, eval = T, fig.align = "center", out.width = "70%", fig.cap = "9 realisations of the posterior RF with constant prior"}
plot_image <- function(l){
  mtrx3d <- data.frame(x = x, y = y, z = ifelse(as.vector(l), "Shale", "Sand"))
  mtrx.melt <- melt(mtrx3d, id.vars = c("x","y"), measure.vars = "z")
  mtrx.melt$value = as.factor(mtrx.melt$value)
...
&= const \times \prod_{i=1,l_i=0}^n \phi_1(d_i;0.02,0.06^2) \prod_{i=1,l_i=1}^n \phi_1(d_i;0.08,0.06^2) \times \exp\Big(\beta\sum_{<i,j>}I(l_i=l_j)\Big).
\end{split}
\end{equation*}

To find the Gibbs formulation of the posterior model, we condition on $\mathbf{d}$ by dividing by $p(\mathbf{d})$, obtaining,
\begin{equation*}
\begin{split}
...
&= const \times \prod_{i=1,l_i=0}^n \phi_1(d_i;0.02,0.06^2) \prod_{i=1,l_i=1}^n \phi_1(d_i;0.08,0.06^2) \times \exp\Big(\beta\sum_{j\in \mathbf{n}_i} I(l_i=l_j)\Big), \\
&i = 1,2,\dots,n.
\end{split}
\end{equation*}
...
## d)
Comparing the two models, we see that the constant prior model has much higher variance in its prediction and a much larger spread in the MMAP and expectation. From the model specifications, it is clear that the Marov RF prior gives a larger probability of two neighbors having equal value. Hence, we get less spread in the prediction and only one large body of shale, as opposed to several smaller areas of shale. By reducing the value of $\beta$, the Markov RF prior model would resemble the constant prior more and get a larger spread.